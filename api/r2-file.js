...
function parseOwnerFromKey(key) {
  // expected: task_submissions/{taskId}/{userId}/{filename}
...
export default async function handler(req, res) {
  try {
    ...
    // Short-lived URL; caller must be authenticated to obtain it.
    const signedUrl = await getSignedUrl(client, getCmd, { expiresIn: 60 * 5 }); // 5 minutes

    // Redirect so <video>/<iframe> can load it without custom headers.
    res.setHeader("Cache-Control", "no-store");
    return res.status(302).setHeader("Location", signedUrl).end();
  } catch (error) {
    console.error("/api/r2-file failed", error);
